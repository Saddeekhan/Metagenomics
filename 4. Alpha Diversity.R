...
# Create a sample data frame with correct "Group" information
sample_data_df <- data.frame(
  SampleID = sample_names(ps),
  Label.Biobank.original.name = rep(NA_character_, nrow(sample_data(ps))),
  Group = c("IE_High_Risk", "Definite_IE", "IE_High_Risk", "Definite_IE", "IE_High_Risk", "Definite_IE", "IE_High_Risk")
)

# Update the "Group" information in the sample_data
sample_data(ps)$Group <- sample_data_df$Group

# Check the first few rows of sample data
head(sample_data(ps))

...
Metadata$SampleID <- make.names(Metadata$SampleID, unique=TRUE)
...
otu_names <- sample_names(otuTABLE)
metadata_names <- rownames(sampleDATA)
setdiff(otu_names, metadata_names)
setdiff(metadata_names, otu_names)

# Modify metadata to match sample names in OTU table
sampleDATA <- sampleDATA[otu_names, ]
rownames(sampleDATA) <- otu_names
